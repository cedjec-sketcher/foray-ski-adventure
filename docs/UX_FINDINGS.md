...
### [blocker] No responsive viewport meta tag — mobile renders as a shrunk desktop page
**Where:** Default (Typical season) view, `resize_window` preset "mobile" (375×812 emulated)
**Observed:** With the viewport emulated at 375×812, `window.innerWidth` reads **980px**, not 375px, and `document.querySelector('meta[name="viewport"]')` returns `null` — there is no `<meta name="viewport">` tag at all. The browser falls back to its default ~980px "layout viewport" and scales the whole desktop two-column layout (map + resort list side by side) down to fit the screen. On an actual phone this means every label, chip, and list row renders far too small to read or tap without the user manually pinch-zooming first.
**Why it matters:** This isn't a layout-polish issue, it's a fundamental "the responsive CSS never gets a chance to run" bug — media queries keyed to viewport width will never fire correctly at real mobile widths. Any phone visitor's first impression is an unusable, tiny page. WCAG 1.4.10 (Reflow) and basic mobile-web baseline both require a working viewport meta tag.
**Suggested direction:** Add `<meta name="viewport" content="width=device-width, initial-scale=1">` to `<head>`, then re-run the mobile check — the existing responsive CSS (chip wrapping, 70vh list cap mentioned in PROPOSALS.md) can only be evaluated once this is in place.
